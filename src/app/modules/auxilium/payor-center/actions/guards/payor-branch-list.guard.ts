...
import { PayorCenterIndividualSelectors } from '../../reducers';
import { PayorDemographicsActions } from '../payor-demographics.action';

@Injectable({
    providedIn: 'root',
})
export class LoadPayorBranchListGuard implements CanActivate {
    constructor(private store: Store<any>) {}

    // Method to load the branch list if it's not already available
    loadBranchList(): Observable<boolean> {
        return this.store.select(PayorCenterIndividualSelectors.selectBranch).pipe(
            take(1),
