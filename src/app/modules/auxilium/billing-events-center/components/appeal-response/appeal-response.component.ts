...
import { AppealResponseAddComponent } from '../appeal-response-add/appeal-response-add.component';
@UntilDestroy()
@Component({
    selector: 'app-appeal-response',
    templateUrl: './appeal-response.component.html',
    styleUrls: ['./appeal-response.component.scss'],
    providers: [DateFormatPipe],
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [AuxAgGridComponent, AsyncPipe],
})
export class AppealResponseComponent implements OnInit, OnDestroy {
    patientId: number;
    rowData = [];
    ...
    columnDefs: ColDef[] = [
        ...
        },
        {
            ...
            onCellClicked: params => this.handleRedirect('appeals', { data: { patientid: params.data.patientId } }),
        },
        {
        ...
        {
            headerName: 'WHAT THESE AN APPEAL FOR A REFUND?',
    ...
    ngOnInit() {
        this.refresh$.pipe(startWith(null), untilDestroyed(this)).subscribe(() => {
            this.store.dispatch(BillingEventsCenterTableActions.LoadBillingEvents({ filter: 'appeals' }));
        });
    ...
    onSelectionChanged(params: CellClickedEvent): void {
        ...
        if (!excludedColumns.includes(params.column.getColId())) {
            this.appealResponseAddEdit(params.data);
        }
    }
    ...
    appealResponseAddEdit(billingevents?: BillingEventsCenterDisplay): void {
        const popupData: PopupData = {
            ...
            saveButtonText: 'Save',
            dynamicComponent: AppealResponseAddComponent,
            dynamicComponentData: billingevents ? { billingevents } : null,
            submitFunction: 'saveAppealResponse',
