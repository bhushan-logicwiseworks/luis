...
import { InsuranceRefundsAddComponent } from '../insurance-refunds-add/insurance-refunds-add.component';
@UntilDestroy()
@Component({
    selector: 'app-insurance-refunds',
    templateUrl: './insurance-refunds.component.html',
    styleUrls: ['./insurance-refunds.component.scss'],
    providers: [DateFormatPipe],
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [AuxAgGridComponent, AsyncPipe],
})
export class InsuranceRefundsComponent implements OnInit, OnDestroy {
    patientId: number;
    rowData = [];
    ...
    columnDefs: ColDef[] = [
        ...
        },
        {
            ...
            onCellClicked: params =>
                this.handleRedirect('insurancerefunds', { data: { patientid: params.data.patientId } }),
        },
        {
        ...
        {
            headerName: 'INS ID BEING REFUNDED',
            field: 'insuranceId',
            filter: 'agMultiColumnFilter',
            minWidth: 150,
        },
    ...
    ngOnInit() {
        this.refresh$.pipe(startWith(null), untilDestroyed(this)).subscribe(() => {
            this.store.dispatch(BillingEventsCenterTableActions.LoadBillingEvents({ filter: 'insurancerefunds' }));
        });
    ...
    onSelectionChanged(params: CellClickedEvent): void {
        ...
        if (!excludedColumns.includes(params.column.getColId())) {
            this.insuranceRefundsAddEdit(params.data);
        }
    }
    ...
    insuranceRefundsAddEdit(billingevents?: BillingEventsCenterDisplay): void {
        const popupData: PopupData = {
            ...
            saveButtonText: 'Save',
            dynamicComponent: InsuranceRefundsAddComponent,
            dynamicComponentData: billingevents ? { billingevents } : null,
            submitFunction: 'saveInsuranceRefunds',
