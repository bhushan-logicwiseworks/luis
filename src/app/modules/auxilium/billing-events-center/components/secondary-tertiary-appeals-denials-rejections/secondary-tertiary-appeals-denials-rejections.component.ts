...
import { SecondaryTertiaryAppealsDenialsRejectionsAddComponent } from '../secondary-tertiary-appeals-denials-rejections-add/secondary-tertiary-appeals-denials-rejections-add.component';

@UntilDestroy()
@Component({
    selector: 'app-secondary-tertiary-appeals-denials-rejections',
    templateUrl: './secondary-tertiary-appeals-denials-rejections.component.html',
    styleUrls: ['./secondary-tertiary-appeals-denials-rejections.component.scss'],
    providers: [DateFormatPipe],
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [AuxAgGridComponent, AsyncPipe],
})
export class SecondaryTertiaryAppealsDenialsRejectionsComponent implements OnInit {
    patientId: number;
    rowData = [];
    ...
    columnDefs: ColDef[] = [
        ...
        },
        {
            ...
            onCellClicked: params => this.handleRedirect('secondary', { data: { patientid: params.data.patientId } }),
        },
        {
        ...
        {
            headerName: 'ACTION THAT WAS TOOK',
            field: 'action',
            filter: 'agMultiColumnFilter',
            minWidth: 170,
        },
        {
            headerName: 'IS THIS A RESULT OF A REFUND?',
            field: 'isRefundResult',
            filter: 'agMultiColumnFilter',
            filterValueGetter: params => (params.data.isRefundResult ? 'YES' : 'NO'),
    ...
    ngOnInit() {
        this.refresh$.pipe(startWith(null), untilDestroyed(this)).subscribe(() => {
            this.store.dispatch(BillingEventsCenterTableActions.LoadBillingEvents({ filter: 'secondary' }));
        });
    ...
    onSelectionChanged(params: CellClickedEvent): void {
        ...
        if (!excludedColumns.includes(params.column.getColId())) {
            this.secondaryAddEdit(params.data);
        }
    }
    ...
    secondaryAddEdit(billingevents?: BillingEventsCenterDisplay): void {
        const popupData: PopupData = {
            ...
            saveButtonText: 'Save',
            dynamicComponent: SecondaryTertiaryAppealsDenialsRejectionsAddComponent,
            dynamicComponentData: billingevents ? { billingevents } : null,
            submitFunction: 'saveSecondary',
