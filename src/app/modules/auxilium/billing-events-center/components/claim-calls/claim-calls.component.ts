...
import { ClaimCallsAddComponent } from '../claim-calls-add/claim-calls-add.component';
@UntilDestroy()
@Component({
    selector: 'app-claim-calls',
    templateUrl: './claim-calls.component.html',
    styleUrls: ['./claim-calls.component.scss'],
    providers: [DateFormatPipe],
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [AuxAgGridComponent, AsyncPipe],
})
export class ClaimCallsComponent implements OnInit, OnDestroy {
    patientId: number;
    rowData = [];
    ...
    columnDefs: ColDef[] = [
        ...
        },
        {
            ...
            onCellClicked: params => this.handleRedirect('claimcalls', { data: { patientid: params.data.patientId } }),
        },
        {
    ...
    visibleColumns = ['id', 'patientId', 'todaysDate', 'claimDateOfService', 'owner', 'insuranceId', 'comments'];
    ...
    ngOnInit() {
        this.refresh$.pipe(startWith(null), untilDestroyed(this)).subscribe(() => {
            this.store.dispatch(BillingEventsCenterTableActions.LoadBillingEvents({ filter: 'claimcalls' }));
        });
    ...
    onSelectionChanged(params: CellClickedEvent): void {
        ...
        if (!excludedColumns.includes(params.column.getColId())) {
            this.claimCallsAddEdit(params.data);
        }
    }
    ...
    claimCallsAddEdit(billingevents?: BillingEventsCenterDisplay): void {
        const popupData: PopupData = {
            ...
            saveButtonText: 'Save',
            dynamicComponent: ClaimCallsAddComponent,
            dynamicComponentData: billingevents ? { billingevents } : null,
            submitFunction: 'saveClaimCalls',
