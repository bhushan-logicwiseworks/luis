...
import { InsurancePaidPatientAddComponent } from '../insurance-paid-patient-add/insurance-paid-patient-add.component';
@UntilDestroy()
@Component({
    selector: 'app-insurance-paid-patient',
    templateUrl: './insurance-paid-patient.component.html',
    styleUrls: ['./insurance-paid-patient.component.scss'],
    providers: [DateFormatPipe],
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [AuxAgGridComponent, AsyncPipe],
})
export class InsurancePaidPatientComponent implements OnInit, OnDestroy {
    patientId: number;
    rowData = [];
    ...
    columnDefs: ColDef[] = [
        ...
        },
        {
            ...
            onCellClicked: params =>
                this.handleRedirect('insurancepaid', { data: { patientid: params.data.patientId } }),
        },
        {
    ...
    ngOnInit() {
        this.refresh$.pipe(startWith(null), untilDestroyed(this)).subscribe(() => {
            this.store.dispatch(BillingEventsCenterTableActions.LoadBillingEvents({ filter: 'insurancepaid' }));
        });
