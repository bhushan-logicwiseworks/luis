...
import { OpenARReportAddComponent } from '../open-ar-report-add/open-ar-report-add.component';
@UntilDestroy()
@Component({
    selector: 'app-open-ar-report',
    templateUrl: './open-ar-report.component.html',
    styleUrls: ['./open-ar-report.component.scss'],
    providers: [DateFormatPipe],
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [AuxAgGridComponent, AsyncPipe],
})
export class OpenArReportComponent implements OnInit, OnDestroy {
    patientId: number;
    rowData = [];
    ...
    columnDefs: ColDef[] = [
        ...
        },
        {
            ...
            onCellClicked: params =>
                this.handleRedirect('openarreport', { data: { patientid: params.data.patientId } }),
        },
        {
    ...
    ngOnInit() {
        this.refresh$.pipe(startWith(null), untilDestroyed(this)).subscribe(() => {
            this.store.dispatch(BillingEventsCenterTableActions.LoadBillingEvents({ filter: 'openarreport' }));
        });
    ...
    onSelectionChanged(params: CellClickedEvent): void {
        ...
        if (!excludedColumns.includes(params.column.getColId())) {
            this.openARReportAddEdit(params.data);
        }
    }
    ...
    openARReportAddEdit(billingevents?: BillingEventsCenterDisplay): void {
        const popupData: PopupData = {
            ...
            saveButtonText: 'Save',
            dynamicComponent: OpenARReportAddComponent,
            dynamicComponentData: billingevents ? { billingevents } : null,
            submitFunction: 'saveOpenARReport',
