...
import { PatientDemographicsSelectors } from '../../reducers';
import { PatientDemographicsActions } from '../patient-demographics.action';

@Injectable({
    providedIn: 'root',
})
export class LoadDemographicsBranchListGuard implements CanActivate {
    constructor(private store: Store<any>) {}

    // Method to load the branch list if it's not already available
    loadBranchList(): Observable<boolean> {
        return this.store.select(PatientDemographicsSelectors.selectBranch).pipe(
            take(1),
