...
@UntilDestroy()
@Component({
    selector: 'app-work-orders-without-pod',
    templateUrl: './work-orders-without-pod.component.html',
    styleUrl: './work-orders-without-pod.component.scss',
    imports: [FuseHorizontalNavigationComponent, AuxAgGridComponent, AsyncPipe],
    providers: [DateTimeFormatPipe, DateFormatPipe],
})
export class WorkOrdersWithoutPodComponent {
    data$ = this.store.select(BillCenterTableSelectors.selectWorkOrdersWithoutPOD);
    ...
    ngOnInit() {
        combineLatest([this.route.paramMap, this.refresh$.pipe(startWith(null))])
            ...
            .subscribe(([paramMap]) => {
                this.store.dispatch(BillCenterTableActions.LoadWorkOrdersWithoutPOD());
            });
