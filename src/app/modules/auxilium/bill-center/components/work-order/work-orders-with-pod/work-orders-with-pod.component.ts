...
@UntilDestroy()
@Component({
    selector: 'app-work-orders-with-pod',
    templateUrl: './work-orders-with-pod.component.html',
    styleUrl: './work-orders-with-pod.component.scss',
    imports: [FuseHorizontalNavigationComponent, AuxAgGridComponent, AsyncPipe],
    providers: [DateTimeFormatPipe, DateFormatPipe],
})
export class WorkOrdersWithPodComponent {
    data$ = this.store.select(BillCenterTableSelectors.selectWorkOrdersWithPOD);
    ...
    ngOnInit() {
        combineLatest([this.route.paramMap, this.refresh$.pipe(startWith(null))])
            ...
            .subscribe(([paramMap]) => {
                this.store.dispatch(BillCenterTableActions.LoadWorkOrdersWithPOD());
            });
