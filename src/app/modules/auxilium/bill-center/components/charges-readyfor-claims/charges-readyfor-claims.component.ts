...
@UntilDestroy()
@Component({
    selector: 'app-charges-readyfor-claims',
    templateUrl: './charges-readyfor-claims.component.html',
    styleUrls: ['./charges-readyfor-claims.component.scss'],
    providers: [DateTimeFormatPipe, DateFormatPipe],
    imports: [FuseHorizontalNavigationComponent, AuxAgGridComponent, AsyncPipe],
})
export class ChargesReadyforClaimsComponent {
    data$ = this.store.select(BillCenterTableSelectors.selectChargesReps);
    ...
    ngOnInit() {
        combineLatest([this.route.paramMap, this.refresh$.pipe(startWith(null))])
            ...
            .subscribe(([paramMap]) => {
                this.store.dispatch(BillCenterTableActions.LoadChargesReadyForClaims());
            });
    ...
    onChangeSelection($event) {
        ...
        const getPostIndex = this.toolbarData.findIndex(res => res.title === 'Convert to Claim');
    ...
    convertToClaim() {
        if (this.selection.isEmpty()) {
            ...
            const confirmation = this._fuseConfirmationService.open({
                title: 'Convert to Claim',
                message: 'Are you sure you want to convert to Claim?',
                actions: {
                    confirm: {
                        label: 'YES',
                    },
            ...
            confirmation
                ...
                .subscribe(result => {
                    if (result === 'confirmed') {
                        ...
                        this.store.dispatch(
                            BillCenterTableActions.Createclaimsfromcharges({ patientData: patientData })
                        );
