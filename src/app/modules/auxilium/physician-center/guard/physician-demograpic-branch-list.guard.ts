...
import { PhysicianCenterIndividualSelectors } from '../reducers';

@Injectable({
    providedIn: 'root',
})
export class LoadPhysicianBranchListGuard implements CanActivate {
    constructor(private store: Store<any>) {}

    // Method to load the branch list if it's not already available
    loadBranchList(): Observable<boolean> {
        return this.store.select(PhysicianCenterIndividualSelectors.selectBranch).pipe(
            take(1),
            switchMap(branchList => {
                if (!branchList || branchList.length === 0) {
                    this.store.dispatch(PhysicianCenterIndividualActions.LoadBranchDropDown());
                }
                return of(true);
            })
        );
